...
import '../Shared Bill Widgets/SaleHistoryCard.dart';
...
class _AddEditBillDialogState extends State<AddEditBillDialog> {
  ...
  final TextEditingController _subtotalAmountController =
  ...
  final TextEditingController _paymentPromisedDateController =
      TextEditingController();
    ...
    showGeneralDialog(
      ...
      pageBuilder: (context, animation1, animation2) {
        return Align(
          ...
          child: Material(
            color:
                Colors.transparent, // Set transparent background for the dialog
            child: Container(
              width:
                  MediaQuery.of(context).size.width * 0.35, // Responsive width
                      ...
                      onTap: () {
                        if (widget.depth <= 5) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content:
                                  Text('Maximum limit of 5 dialogs reached!')));
                        }
                      },
                    );
                  }),
  ...
  Widget _optionTile({
    ...
    return InkWell(
      // Replaced GestureDetector with InkWell to add visual ripple effect
      onTap: onTap,
      borderRadius:
          BorderRadius.circular(12), // Ensure ripple effect fits the border
  ...
  void initState() {
    ...
    if (widget.bill != null) {
      ...
      _paymentPromisedDateController.text =
          DateFormat('dd-MM-yyyy').format(promiseDate);
      ...
      _selectedItems = widget.bill!.items.map((item) {
        ...
        _discountControllers[item.itemId] = TextEditingController(
            text: item.itemDiscount.toStringAsFixed(
                2)); // Initialize with 0.00 or fetch it if applicable
        _saleRateControllers[item.itemId] =
            TextEditingController(text: item.saleRate.toStringAsFixed(2));
  ...
  void _setDefaultPromisedDate() {
    DateTime pickedDefaultDate = DateTime.now();
    _paymentPromisedDateController.text =
        DateFormat('dd-MM-yyyy').format(pickedDefaultDate);
  }
  ...
  Future<void> _fetchCustomerDetails() async {
    if (_selectedCustomerId != null) {
      ...
      final allBills =
          await _billService.getBillsByCustomerId(_selectedCustomerId!);

      // Filter bills to show only SaleBill type
      final saleBills =
          allBills.where((bill) => bill.billType == 'Sale Bill').toList();

      setState(() {
        _selectedCustomer = customer;
        _previousBills =
            saleBills.take(5).toList(); // Show only the recent 5 SaleBills
    ...
    if (item.availableQuantity > 0) {
      ...
      setState(() {
        ...
        if (!_saleRateControllers.containsKey(item.id)) {
          _saleRateControllers[item.id] =
              TextEditingController(text: item.saleRate.toString());
        }
  ...
  void _updateItemQuantity(BillItem item, int quantity) {
    if (quantity <=
        _items.firstWhere((i) => i.id == item.itemId).availableQuantity) {
      setState(() {
  ...
  Future<void> _handleSave(
      {bool printBill = false, BusinessDetails? bussinessDetails}) async {
    setState(() {
      _isLoading = true;
    });
...
print("Date:$date");
    ...
    if (printBill) {
      ...
      BillPdfGenerator.generatePdfAndView(
          newBill, customer, newBill.billType, context ,bussinessDetails!);
    }
...
// Function to update customer balance for a new bill
  Future<void> _updateCustomerBalanceForNewBill(double creditAmount) async {
    ...
    print(
        'Customer balance updated for ID: $_selectedCustomerId to $newBalance');
  }

// Function to update customer balance for an existing bill
  Future<void> _updateCustomerBalanceForExistingBill(
      double creditAmount) async {
    if (_selectedStatus == 'Non Completed') {
      ...
      final previousTotal =
          widget.bill!.totalAmount; // Previous total from the bill
      final previousCredit =
          widget.bill!.amountGiven; // Previous credit from the bill

      // Deduct previous total and credit from balance (because we are now marking it as non-completed)
      final newBalance =
          _selectedCustomer!.balance - previousTotal + previousCredit;

      // Update the customer balance
      await _billService.updateCustomerBalance(
          _selectedCustomerId!, newBalance);
      print(
          'Customer balance updated for ID: $_selectedCustomerId with Non Completed status. Previous total and credit removed. New balance: $newBalance');
    } else {
      ...
      print(
          'Customer balance updated for ID: $_selectedCustomerId with combined changes. New balance: $newBalance');

      if (_totalAmount == previousTotal && creditAmount == previousCredit) {
        print(
            'No balance update needed as both total and credit amounts are unchanged.');
      }
    }
  }
  ...
  void compareBills(Bill oldBill, Bill newBill) {
    ...
    if (oldBill.totalAmount != newBill.totalAmount) {
      print(
          'Total amounts are different: old = ${oldBill.totalAmount}, new = ${newBill.totalAmount}');
    }
    if (oldBill.discount != newBill.discount) {
      print(
          'Discounts are different: old = ${oldBill.discount}, new = ${newBill.discount}');
    }
    ...
    if (oldBill.status != newBill.status) {
      print(
          'Statuses are different: old = ${oldBill.status}, new = ${newBill.status}');
    }
    if (oldBill.date != newBill.date) {
      print(
          'Dates are different: old = ${oldBill.date}, new = ${newBill.date}');
    }
  ...
  void compare_updateItemListsFromDB(
      List<BillItem> oldList, List<BillItem> newList) {
    // Create maps to store items by their ID or name
    Map<String, BillItem> oldItemMap = {
      for (var item in oldList) item.itemId: item
    };
    Map<String, BillItem> newItemMap = {
      for (var item in newList) item.itemId: item
    };
    ...
    for (var item in oldList) {
      if (!newItemMap.containsKey(item.itemId)) {
        ...
        _billService.updateItemQuantity(item.itemId, item.quantity,
            increase: true);
        ...
        if (newItem.quantity != item.quantity) {
          ...
          */
          ...
          _billService.updateItemQuantity(
            item.itemId,
            quantityDifference
                .abs(), // Use the absolute value of the difference
            increase: shouldIncrease,
          );

          print(
              'Item quantity changed for ${item.name}: old = ${item.quantity}, new = ${newItem.quantity}');
        }
        if (newItem.saleRate != item.saleRate) {
          print(
              'Sale rate changed for ${item.name}: old = ${item.saleRate}, new = ${newItem.saleRate}');
        }
        if (newItem.purchaseRate != item.purchaseRate) {
          print(
              'Purchase rate changed for ${item.name}: old = ${item.purchaseRate}, new = ${newItem.purchaseRate}');
        }
        if (newItem.total != item.total) {
          print(
              'Total changed for ${item.name}: old = ${item.total}, new = ${newItem.total}');
        }
      }
    }
  ...
  void _showItemSelectionDialog() async {
    final items =
        await _billService.getItems(); // Fetch items from your service
    showDialog(
      context: context,
  ...
  String _calculateProfit() {
    ...
    double totalPurchasePrice =
        _calculateTotalPurchasePrice(); // Assuming you have this method to calculate total purchase price.
                    ...
                    Container(
                      ...
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment
                            .spaceBetween, // Align items with space between
                        children: [
                          IconButton(
                          ...
                          SizedBox(
                              width: 48), // Empty SizedBox to balance the space
                        ],
                      ),
                    ),
                    SizedBox(height: 16),
                                          ...
                                          ElevatedButton(
                                            ...
                                            style: ElevatedButton.styleFrom(
                                              backgroundColor: Colors
                                                  .blue, // Set button color if needed
                                            ),
                                          ),
                                        ],
                                      ),
                                            ...
                                            children: [
                                              businessProvider.businessDetails!
                                                  ...
                                                  : SizedBox(
                                                      ...
                                                      height: 70,
                                                      child: Image.network(
                                                          '${dotenv.env['BACKEND_URL']!}${businessProvider.businessDetails!.companyLogo}'),
                                                    ),
                                              Text(
                                                businessProvider
                                                    .businessDetails!
                                                ...
                                                TypeAheadFormField<Customer>(
                                                  ...
                                                  suggestionsCallback:
                                                    ...
                                                    return _customers.where((customer) =>
                                                        ...
                                                        customer.id.contains(
                                                            pattern)); // Limit to 5 suggestions
                                                  },
                                                  itemBuilder: (context,
                                                      Customer suggestion) {
                                                    return Container(
                                                  ...
                                                  suggestionsBoxDecoration:
                                                    ...
                                                    constraints: BoxConstraints(
                                                      maxHeight:
                                                          225, // Control the height of the suggestions box
                                                    ),
                                                  ),
                                                ),
                                                SizedBox(height: 16),
                                                ...
                                                CustomTextField(
                                                  ...
                                                  inputFormatters: [
                                                    FilteringTextInputFormatter
                                                        .allow(RegExp(
                                                            r'^\d*\.?\d{0,2}')), // Custom formatter for decimals
                                                  ],
                                                  onChanged: (value) {
                                                    setState(() {
                                                      _customerPhoneController
                                                ...
                                                onChanged: (value) {
                                                  ...
                                                  setState(() {
                                                    ...
                                                    _updateItemRateForBill(
                                                        ...
                                                        _selectedItems[i]
                                                            .saleRate);
                                                    _saleRateControllers[
                                                                _selectedItems[
                                                                        i]
                                                                    .itemId]!
                                                            .text =
                                                        newSaleRate.toString();
                                                  });
                                                },
                                              ),
                                            ),
                                        ...
                                        _selectedStatus == 'Non Completed'
                                            ...
                                            : Column(
                                                ...
                                                children: [
                                                    ...
                                                    children: [
                                                      ...
                                                      Text(
                                                        ...
                                                        style: TextStyle(
                                                          ...
                                                          fontSize: 20,
                                                          color:
                                                              _getProfitColor(), // Green for profit, red for loss
                                                        ),
                                                      ),
                                                    ],
                                                  ),
                                                  ...
                                                  SizedBox(
                                                    width:
                                                        300, // Set the width you want
                                                    child: TextField(
                                                      controller:
                                                          _descriptionController, // Your description controller
                                                      maxLines:
                                                          3, // Limit to 3 lines
                                                      textAlign: TextAlign
                              ...
                              children: [
                                ...
                                if (_selectedCustomer != null &&
                                    _customerNameController.text
                                        .trim()
                                        .isNotEmpty &&
                                    _previousBills.isNotEmpty)
                                  PreviousCustomerBillsCard(
                                    previousBills: _previousBills,
                                  ),

                                // Customer previous purchase Card
                                if (_selectedIndex != -1 &&
                                    _selectedIndex < _selectedItems.length &&
                                    _selectedItems.isNotEmpty)
                                  SaleInformationCard(
                                      previousBills: _previousBills,
                                      selectedItem:
                                          _selectedItems[_selectedIndex]),
                                ...
                                if (_selectedIndex != -1 &&
                                  ...
                                  SalesHistoryCard(
                                    itemId:
                                        _selectedItems[_selectedIndex].itemId,
                                    selectedItem:
                                        _selectedItems[_selectedIndex],
                                  ),
                                  ...
                                  // PurchaseHistoryCard(
                                  //     previousPurchaseRates:
                                  //         _itemPreviousPurchaseRates[
                                  //                 _selectedItems[_selectedIndex]
                                  //                     .itemId] ??
                                  //             [],
                                  //     selectedItem:
