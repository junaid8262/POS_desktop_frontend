...
class AddEditReturnBillDialog extends StatefulWidget {
  ...
  final Function(String billId)?
      onBillSavedReturnBillId; // Updated callback function

  const AddEditReturnBillDialog(
      {Key? key,
      this.bill,
      this.onBillSavedReturnBillId,
      required this.onBillSaved})
      : super(key: key);

  @override
  _AddEditReturnBillDialogState createState() =>
      _AddEditReturnBillDialogState();
}
...
class _AddEditReturnBillDialogState extends State<AddEditReturnBillDialog> {
  ...
  Future<void> _fetchCustomerDetails() async {
    if (_selectedCustomerId != null) {
      ...
      final filteredBills = allBills
          .where((bill) =>
      bill.status == 'Completed' && bill.billType == 'Sale Bill')
          .toList();

      // Sort the filtered bills in descending order by date
      filteredBills.sort((a, b) => b.date.compareTo(a.date));

      setState(() {
        _selectedCustomer = customer;
        _previousBills = filteredBills.take(5).toList(); // Get only the recent 10 filtered bills
  ...
  void _addItemToBill(Item item, double discount) async {
    ...
    setState(() {
      ...
      if (existingItem != null) {
        ...
        int initialQuantity = item
            .availableQuantity; // or use the quantity from the previous bill if applicable
        _selectedItems.add(BillItem(
          ...
          purchaseRate: item.purchaseRate,
          total: item.saleRate *
              initialQuantity, // Total for the initial quantity
          miniUnit: item.miniUnit,
          item: item, saleRate: item.saleRate,
          itemDiscount: discount, // Store the item reference
        ));
        _quantityControllers[item.id]!.text = initialQuantity
            .toString(); // Set the controller to the initial quantity
      }
    ...
    setState(() {
      ...
      if (quantity > (billItem.item?.availableQuantity ?? 0)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                'Cannot return more than available quantity: ${billItem.item?.availableQuantity}')));
        return; // Exit the method if the quantity is invalid
      }

      billItem.quantity = quantity;
      billItem.total = billItem.saleRate * quantity;
  ...
  Future<void> _handleSave(
    ...
    try {
      ...
      if (printBill) {
        ...
        final customer =
            await _billService.getCustomerById(_selectedCustomerId!);
  ...
  Future<void> _updateCustomerBalanceForNewBillReversed() async {
    ...
    print(
        'Customer balance updated for ID: $_selectedCustomerId with new bill. Balance subtracted. New balance: $newBalance');
  }
...
// Function to update customer balance for an existing bill (Reversed logic)
  Future<void> _updateCustomerBalanceForExistingBillReversed() async {
    ...
    print(
        'Customer balance updated for ID: $_selectedCustomerId with reversed changes. New balance: $newBalance');
            ...
            children: [
              ...
              pw.Container(
                padding: const pw.EdgeInsets.symmetric(
                    vertical: 15), // Adjust padding for more vertical spacing
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment
                      .start, // Align all content to the left for consistency
                  children: [
                      ...
                      children: [
                        pw.Column(
                          ...
                          children: [
                            ...
                            pw.SizedBox(
                                height:
                                    3), // Minimal space between label and value
              ...
              pw.Table.fromTextArray(
                headers: ['#', 'Product Name', 'Qty', 'Sale Price', 'Total'],
                data: _selectedItems.asMap().entries.map((entry) {
                  final index = entry.key + 1;
                  final item = entry.value;
                  final total =
                      (item.saleRate * item.quantity).toStringAsFixed(2);
                  return [
                    index.toString(),
                    item.name,
  ...
  void _onItemsReturned(List<Map<String, dynamic>> selectedItems) {
    ...
    for (var itemMap in selectedItems) {
      ...
      print(
          'Item: ${item.name}, Quantity: ${item.availableQuantity}, Sale Rate: \$${item.saleRate}, discount Value: \$${discount}');
    }
  }
                                    ...
                                    textFieldConfiguration:
                                      ...
                                      onTap: () {
                                        ...
                                        _customerNameController.selection =
                                            TextSelection(
                                          baseOffset: 0,
                                          extentOffset: _customerNameController
                                              .text.length,
                                        );
                                      },
                                    ),
                                                        ...
                                                        setState(() {
                                                          if (quantity <=
                                                            ...
                                                            _updateItemQuantity(
                                                                _selectedItems[
                                                                    i],
                                                                quantity); // Update only the quantity
                                                          } else {
                                                ...
                                                ElevatedButton(
                                                  ...
                                                  style:
                                                    ...
                                                    backgroundColor: Colors
                                                        .red, // Set the button background color to red for return bills
                                                  ...
                                                  child: Row(
                                                    children: [
                                                      Icon(Icons.save,
                                                          size: 18,
                                                          color: Colors
                                                              .white), // Add an icon to improve UX
                                                      SizedBox(width: 8),
                                                      Text(
                                                        'Save Return Bill',
                                                        style: TextStyle(
                              ...
                              children: [
                                ...
                                if (_selectedCustomer != null &&
                                  ...
                                  BillSearchAndDetailsScreen(
                                    ...
                                    onItemsSelected: _onItemsReturned,
                                  ),
