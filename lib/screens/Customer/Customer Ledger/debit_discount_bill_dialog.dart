...
class _BillDialogState extends State<BillDialog> {
  ...
  void _handleAmountChange(String value) {
    ...
    if (enteredAmount > widget.balance) {
      setState(() {
        _warningMessage = widget.isDebitBill ? 'Debit amount cannot exceed balance!' : 'Discount amount cannot exceed balance!';
      });
    } else {
      setState(() {
        _warningMessage = '';
  ...
  Future<void> _addDebitAmount(double debitAmount, String description, DateTime date , bool isDebitBill) async {
    ...
    try {
      ...
      // Create a new bill for the debit amount
      String billId = await _billService.addBill(
        Bill(
          id: '',
      ...
      if (widget.onBillSavedReturnBillId != null) {
        widget.onBillSavedReturnBillId!(billId);
      }
      // Update customer balance
      double newBalance = widget.balance - debitAmount;
      print ("debit/discount amount ${debitAmount}");
      print ("customer balance is ${widget.balance}");
      print ("new balance is ${newBalance}");
       await _billService.updateCustomerBalance(widget.customer.id, newBalance);
